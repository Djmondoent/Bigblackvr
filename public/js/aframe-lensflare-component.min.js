!(function(e) {
  ...
  function(e, t, i) {
    ...
    if ("undefined" == typeof THREE)
      throw new Error(
        "Component attempted to register before THREE was available."
      );
    i(1),
      AFRAME.registerComponent("lensflare", {
        schema: {
          src: { type: "asset" },
          createLight: { type: "boolean", default: !0 },
          position: { type: "vec3" },
          target: { type: "string" },
          intensity: { type: "number", default: 5 },
          relative: { type: "boolean", default: !0 },
          size: { type: "number", default: 500 },
          lightColor: { type: "string", default: "rgb(255, 255, 255)" },
          lightDistance: { type: "number", default: 4 },
          lightAngle: { type: "number", default: Math.PI / 3 },
          lightPenumbra: { type: "number", default: 0.077 },
          lightDecay: { type: "number", default: 1 },
          lightType: {
            default: "spot",
        ...
        init: function() {
          ...
          if (
            ...
            (this.lensFlareElement = new THREE.LensflareElement(
              a,
              this.data.size,
              0,
              new THREE.Color(this.data.lightColor)
            )),
            this.lensFlare.addElement(this.lensFlareElement),
            this.lensFlare.position.copy(r),
            this.data.createLight)
          ) {
            var o = this.setLightType(
                this.data.lightType.toLowerCase(),
                this.data
              ),
              l = !!this.data.target && this.data.target;
            l &&
              ((o.target = document.querySelector(this.data.target).object3D),
              i.add(o.target),
              i.updateMatrixWorld()),
              o.position.set(r.x, r.y, r.z),
              this.data.relative
                ? (o.add(this.lensFlare), t.add(o), i.updateMatrixWorld())
                : e.add(o);
          } else
            this.data.relative
              ? (t.add(this.lensFlare), i.updateMatrixWorld())
              : e.add(this.lensFlare);
        },
        update: function(e) {},
        remove: function() {},
        pause: function() {},
  ...
  function(e, t) {
    ...
    (THREE.Lensflare = function() {
      THREE.Mesh.call(
        this,
        THREE.Lensflare.Geometry,
        new THREE.MeshBasicMaterial({ opacity: 0, transparent: !0 })
      ),
        (this.type = "Lensflare"),
      ...
      (this.onBeforeRender = function(c, h, m) {
        ...
        if (
          (E.set(R * y, R),
          d.min.set(f.x, f.y),
          d.max.set(f.x + (f.z - 16), f.y + (f.w - 16)),
          t.setFromMatrixPosition(this.matrixWorld),
          t.applyMatrix4(m.matrixWorldInverse),
          !(t.z > 0) &&
            (e.copy(t).applyMatrix4(m.projectionMatrix),
            (v.x = f.x + e.x * g + g - 8),
            (v.y = f.y + e.y * T + T - 8),
            d.containsPoint(v)))
        ) {
          ...
          for (
            var w = 2 * -e.x, H = 2 * -e.y, b = 0, M = s.length;
            b < M;
            b++
          ) {
            var D = s[b],
        ...
        (this.dispose = function() {
          a.dispose(), o.dispose(), u.dispose(), i.dispose(), r.dispose();
          for (var e = 0, t = s.length; e < t; e++) s[e].texture.dispose();
        });
    }),
      ...
      (THREE.Lensflare.Geometry = (function() {
        ...
        return (
          ...
          e.addAttribute(
            "position",
            new THREE.InterleavedBufferAttribute(i, 3, 0, !1)
          ),
          e.addAttribute(
            "uv",
            new THREE.InterleavedBufferAttribute(i, 2, 3, !1)
          ),
          e
        );
      })());
