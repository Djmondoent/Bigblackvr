(function(global, factory) {
  ...
  AFRAME.registerComponent("thumb-controls", {
    ...
    init: function() {
      ...
      el.addEventListener("controllerconnected", evt => {
        this.axis = el.components["tracked-controls"].axis;
        if (
          evt.detail.name === "oculus-touch-controls" ||
    ...
    onTrackpadDown: function() {
      var direction;
      var el = this.el;
      if (this.getDistance() < this.data.thresholdPad) {
        return;
      }
    ...
    getDirection: function() {
      ...
      for (i = 0; i < ANGLES.length; i++) {
        ...
        if (topThreshold > 360) {
          topThreshold = topThreshold - 360;
        }
        bottomThreshold = ANGLES[i] - threshold;
        if (bottomThreshold < 0) {
          if (
            (angle >= 360 + bottomThreshold && angle <= 360) ||
  ...
  AFRAME.registerComponent("thumb-controls-debug", {
    ...
    init: function() {
      ...
      var el = this.el;
      var data = this.data;
      if (!data.enabled && !AFRAME.utils.getUrlParameter("debug-thumb")) {
        return;
      }
