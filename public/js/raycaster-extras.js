...
AFRAME.registerComponent("raycaster-extras", {
  schema: {
    ...
    beamLength: { type: "float", default: 0.4 },
    beamColor: { type: "color", default: "white" },
    beamOpacity: { type: "float", default: 0.5 },
    beamImageSrc: { type: "string", default: "#gradient" },
    cursorRadius: { type: "float", default: 0.02 },
  ...
  init: function () {
    ...
    this.beamEntity.setAttribute(
      "material",
      "shader: flat; transparent: true; repeat: 2 1;"
    );
    ...
    this.cursorBorder.setAttribute(
      "material",
      "shader: flat; color: black; side: back;"
    );
    ...
    this.cursorCenter.setAttribute("radius", this.data.cursorRadius * 0.8);
  ...
  tick: function () {
    ...
    if (
      ...
      this.beamEntity.object3D.children[0].material.color =
        this.beamColorActive;
    // setAttribute("material", "color", "cyan");
    else
      this.beamEntity.object3D.children[0].material.color =
        this.beamColorDefault; // setAttribute("material", "color", this.data.beamColor);
    ...
    if (
      ...
      this.beamAngleX =
        180 +
        (Math.atan2(this.currentRayDirection.z, this.currentRayDirection.y) *
          180) /
          Math.PI;
      ...
      this.pos = {
        x: this.currentRayOrigin.x,
        y:
          this.currentRayOrigin.y -
          Math.cos(this.angleRad) * this.cylinderShift,
        z:
          this.currentRayOrigin.z -
          Math.sin(this.angleRad) * this.cylinderShift,
      };
    ...
    if (
      ...
      this.intersectionPoint =
        this.raycaster.intersectionDetail.intersections[0].point;
    } else {
      this.focusedElement = null;
      this.intersectionPoint = null;
    }
    ...
    if (
      this.controllerData.rightGrip.pressed &&
      this.focusedElement != null &&
      this.grabbedElement == null
    ) {
      // if it can be grabbed...
      if (
        this.focusedElement.components["raycaster-target"] &&
        ...
        // emit an event
        this.grabbedElement.emit("raycaster-grabbed", {
          el: this.grabbedElement,
        });
      }
    }
    ...
    if (this.grabbedElement != null) {
      ...
      if (this.controllerData.rightAxisY != 0) {
        ...
        if (!(this.controllerData.rightAxisY > 0 && distance < 0.05)) {
          this.moveDistance =
            ...
            this.moveSpeed *
            this.deltaTime *
            this.controllerData.rightAxisY;
          ...
          this.grabbedElement.sceneEl.object3D.attach(
            this.grabbedElement.object3D
          );
          // translate
      ...
      } // if not pushing/pulling, revert beam graphics to solid line ------------------------------
      else {
        let material = this.beamEntity.getAttribute("material");
        material.repeat.x = 2;
        material.repeat.y = 1;
      ...
      if (this.controllerData.rightGrip.released) {
        ...
        // emit an event
        this.grabbedElement.emit("raycaster-released", {
          el: this.grabbedElement,
        });
