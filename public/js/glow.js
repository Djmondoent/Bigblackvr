AFRAME.registerComponent("glow", {
  ...
  update: function () {
    if (this.data.enabled) {
      if (this.glowMesh) {
        ...
        this.glowMesh.material.uniforms.glowColor.value.setHex(
          this.data.color.replace("#", "0x")
        );
  ...
  tick: function () {
    if (this.glowMesh) {
      this.glowMesh.rotation.set(
        this.el.object3D.rotation.x,
        this.el.object3D.rotation.y,
        this.el.object3D.rotation.z
      );
      this.glowMesh.scale.set(
        this.el.object3D.scale.x * this.data.scale,
        this.el.object3D.scale.y * this.data.scale,
        this.el.object3D.scale.z * this.data.scale
      );
      if (!this.camera) {
        return;
      }
...
TypedArrayHelper.prototype = {
  ...
  index_to_register: function (index, register, isLoop) {
    ...
    if (register >= this.available_registers) {
      throw new Error(
        "THREE.BufferSubdivisionModifier: Not enough registers in TypedArrayHelper."
      );
    }
...
function convertGeometryToIndexedBuffer(geometry) {
  ...
  var vertArray = new TypedArrayHelper(
  ...
  var indexArray = new TypedArrayHelper(
  ...
  var uvArray = new TypedArrayHelper(
    geometry.faceVertexUvs[0].length * 3 * 3,
    0,
    THREE.Vector2,
    Float32Array,
    2,
  ...
  BGeom.addAttribute(
    "position",
    new THREE.BufferAttribute(vertArray.buffer, 3)
  );
...
function compute_vertex_normals(geometry) {
  ...
  var oldVertices = new TypedArrayHelper(
    0,
    5,
    THREE.Vector3,
    Float32Array,
  ...
  var newNormals = new TypedArrayHelper(
    oldVertices.length * 3,
    4,
    THREE.Vector3,
    Float32Array,
    3,
  ...
  var newNormalFaces = new TypedArrayHelper(
    oldVertices.length,
    1,
    function () {
      this.x = 0;
  ...
  for (var i = 0, il = oldFaces.length; i < il; i++) {
    ...
    newNormals.register[0].subVectors(
      oldVertices.register[1],
      oldVertices.register[0]
    );
    newNormals.register[1].subVectors(
      oldVertices.register[2],
      oldVertices.register[1]
    );
  ...
  for (var i = 0, il = oldFaces.length; i < il; i++) {
    ...
    newNormals.register[3].x =
      newNormals.register[0].y * newNormals.register[1].z -
      newNormals.register[0].z * newNormals.register[1].y;
    newNormals.register[3].y =
      newNormals.register[0].z * newNormals.register[1].x -
      newNormals.register[0].x * newNormals.register[1].z;
    newNormals.register[3].z =
      newNormals.register[0].x * newNormals.register[1].y -
      newNormals.register[0].y * newNormals.register[1].x;
    ...
    newNormals.buffer[oldFaces.register[0].a * 3 + 0] +=
      newNormals.register[3].x * full_weights[0];
    newNormals.buffer[oldFaces.register[0].a * 3 + 1] +=
      newNormals.register[3].y * full_weights[0];
    newNormals.buffer[oldFaces.register[0].a * 3 + 2] +=
      newNormals.register[3].z * full_weights[0];

    newNormals.buffer[oldFaces.register[0].b * 3 + 0] +=
      newNormals.register[3].x * full_weights[1];
    newNormals.buffer[oldFaces.register[0].b * 3 + 1] +=
      newNormals.register[3].y * full_weights[1];
    newNormals.buffer[oldFaces.register[0].b * 3 + 2] +=
      newNormals.register[3].z * full_weights[1];

    newNormals.buffer[oldFaces.register[0].c * 3 + 0] +=
      newNormals.register[3].x * full_weights[2];
    newNormals.buffer[oldFaces.register[0].c * 3 + 1] +=
      newNormals.register[3].y * full_weights[2];
    newNormals.buffer[oldFaces.register[0].c * 3 + 2] +=
      newNormals.register[3].z * full_weights[2];
  }

  newNormals.trim_size();
  geometry.addAttribute(
    "normal",
    new THREE.BufferAttribute(newNormals.buffer, 3)
  );
}
...
function unIndexIndexedGeometry(geometry) {
  ...
  var oldNormals = new TypedArrayHelper(
    0,
    3,
    THREE.Vector3,
    Float32Array,
  ...
  var newVertices = new TypedArrayHelper(
    oldFaces.length * 3,
    3,
    THREE.Vector3,
    Float32Array,
  ...
  var newUvs = new TypedArrayHelper(
    oldFaces.length * 3,
    3,
    THREE.Vector2,
    Float32Array,
  ...
  geometry.addAttribute(
    "position",
    new THREE.BufferAttribute(newVertices.buffer, 3)
  );
  geometry.addAttribute(
    "normal",
    new THREE.BufferAttribute(newNormals.buffer, 3)
...
(function () {
  ...
  THREE.BufferSubdivisionModifier.prototype.smooth = function (geometry) {
    ...
    oldVertices = new TypedArrayHelper(
      0,
      3,
      THREE.Vector3,
      Float32Array,
    ...
    newVertices = new TypedArrayHelper(
      (geometry.getAttribute("position").array.length * 2) / 3,
      2,
      THREE.Vector3,
      Float32Array,
      3,
    ...
    for (i in sourceEdges) {
      ...
      newEdge
        .addVectors(oldVertices.register[0], oldVertices.register[1])
      ...
      for (j = 0; j < connectedFaces; j++) {
        ...
        for (k = 0; k < 3; k++) {
          ...
          if (
            face[ABC[k]] !== currentEdge.a &&
            face[ABC[k]] !== currentEdge.b
          ) {
            break;
          }
        }
    ...
    newFaces = new TypedArrayHelper(
      (geometry.index.array.length * 4) / 3,
      1,
      THREE.Face3,
      Float32Array,
      3,
    ...
    newUVs = new TypedArrayHelper(
      (geometry.getAttribute("uv").array.length * 4) / 2,
      3,
      THREE.Vector2,
      Float32Array,
      2,
