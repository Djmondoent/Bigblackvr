AFRAME.registerComponent("player-move", {
  ...
  init: function () {
    ...
    if (document.querySelector(this.data.raycasterId))
      this.raycaster = document.querySelector(this.data.raycasterId).components[
        "raycaster"
      ];
    else this.raycaster = null;
  ...
  tick: function () {
    ...
    if (
      ...
      this.raycaster.intersectionDetail.els[0].classList.contains(
        this.data.navigationMeshClass
      )
    ) {
      ...
      this.navTarget1.object3D.position.set(
        this.point.x,
        this.point.y,
        this.point.z
      );
      ...
      this.alpha = Math.min(2 * this.t, -0.5);
      this.navTarget2.object3D.position.set(
        this.point.x,
        this.point.y,
        this.point.z
      );
    ...
    if (this.fadeInProgress) {
      ...
      this.alpha =
        -Math.abs(this.fadeTime - this.fadeDuration) / this.fadeDuration + 1;
      this.fadeSphere.setAttribute("material", "opacity", this.alpha);

      if (this.fadeTime >= this.fadeDuration) {
        this.el.object3D.position.set(
          this.teleportPoint.x,
          this.teleportPoint.y,
          this.teleportPoint.z
        );
      }
    ...
    if (
      ...
      this.leftJoystickAngle = Math.atan2(
        this.controllerData.leftAxisY,
        this.controllerData.leftAxisX
      );
      ...
      this.moveDistance *= 4.5 + 14 * this.controllerData.leftTrigger.value;
      ...
      this.moveRight =
        -this.leftJoystickLength * Math.sin(this.moveAngle) * this.moveDistance;
      this.moveForward =
        this.leftJoystickLength * Math.cos(this.moveAngle) * this.moveDistance;

      this.el.object3D.position.x =
        this.el.object3D.position.x + this.moveRight;
      this.el.object3D.position.z =
        this.el.object3D.position.z + this.moveForward;
    }
    ...
    if (Math.abs(this.leftX) < 0.31 && Math.abs(this.rightX) < 0.31) {
      this.turnReady = true;
    }
    ...
    if (
      ...
      ((this.controllerData.leftGrip.pressing && Math.abs(this.leftX) > 0.9) ||
        Math.abs(this.rightX) > 0.99)
    ) {
      this.startAngle = this.el.getAttribute("rotation").y;

      if (this.leftX > 0.99 || this.rightX > 0.99)
    ...
    if (
      ...
      // move faster if pressing trigger at same time
      this.moveDistance *= 6.5 + 14 * this.controllerData.leftTrigger.value;

      this.el.object3D.position.y =
        this.el.object3D.position.y + this.moveDistance;
    }
  },
});
