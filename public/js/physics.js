...
Util.positionObject3DAtTarget = function (
  obj,
  target,
  { scale, transformOffset, transformRoot } = {}
) {
...
Util.whenComponentInitialized = function (el, component, fn) {
  ...
  return new Promise((r, e) => {
    if (
      el &&
      el.components[component] &&
      el.components[component].initialized
    ) {
      return Promise.resolve(fn ? fn() : undefined);
    }
...
AFRAME.registerSystem("physx", {
  ...
  startPhysXScene() {
    ...
    const physxSimulationCallbackInstance =
      PhysX.PxSimulationEventCallback.implement({
  ...
  setupDefaultEnvironment() {
    ...
    this.defaultFilterData = new PhysX.PxFilterData(
      PhysXUtil.layersToMask(this.data.groundCollisionLayers),
      PhysXUtil.layersToMask(this.data.groundCollisionMask),
      0,
      0
    );
  ...
  async registerComponentBody(component, { type }) {
    ...
    for (let shape of component.createShapes(
      this.physics,
      this.defaultActorFlags
    )) {
      body.attachShape(shape);
      ...
      if (isFinite(shape.density)) {
        ...
        if (seenAnyDensity) {
          console.warn(
            "Densities not set for all shapes. Will use total mass instead.",
            component.el
          );
        }
      }
  ...
  tock(t, dt) {
    ...
    if (!this.physXInitialized && this.data.autoLoad && !this.running)
      this.startPhysX();
    ...
    this.scene.simulate(
      THREE.Math.clamp(
        (dt * this.data.speed) / 1000,
        0,
        0.03 * this.data.speed
      ),
      true
    );
    ...
    this.cumTime += endTime;
    this.cumTime -= startTime;
    this.tickCounter++;
...
AFRAME.registerComponent("physx-body", {
    ...
    stateremoved: function (e) {
      if (e.detail === "grabbed") {
        ...
        if (this.data.type !== "kinematic") {
          this.rigidBody.setRigidBodyFlag(
  ...
  init() {
    ...
    this.physxRegisteredPromise = this.system.registerComponentBody(this, {
      type: this.data.type,
    });
  ...
  update(oldData) {
    ...
    if (!oldData || this.data.mass !== oldData.mass)
      this.el.emit("object3dset", {});
  },
  remove() {
  ...
  createGeometry(o) {
    if (o.el.hasAttribute("geometry")) {
      ...
      switch (geometry.primitive) {
        case "sphere":
          return new PhysX.PxSphereGeometry(
            geometry.radius * this.el.object3D.scale.x * 0.98
          );
        case "box":
  ...
  createConvexMeshGeometry(mesh, rootAncestor) {
    ...
    let convexMesh = this.system.cooking.createConvexMesh(
      vectors,
      this.system.physics
    );
    return new PhysX.PxConvexMeshGeometry(
      ...
      new PhysX.PxConvexMeshGeometryFlags(
        PhysX.PxConvexMeshGeometryFlag.eTIGHT_BOUNDS.value
      )
    );
  },
  createShape(physics, geometry, materialData) {
    let material = physics.createMaterial(
      ...
      materialData.restitution
    );
    let shape = physics.createShape(
      geometry,
      material,
      false,
      this.system.defaultActorFlags
    ...
    shape.setQueryFilterData(
      new PhysX.PxFilterData(
        PhysXUtil.layersToMask(materialData.collisionLayers),
        PhysXUtil.layersToMask(materialData.collidesWithLayers),
        materialData.collisionGroup,
        0
      )
    );
    ...
    Util.traverseCondition(
      ...
      (o) => {
        ...
        if (
          ...
          !o.el.hasAttribute("physx-hidden-collision")
        )
          return false;
        if (!o.visible && o.el && !o.el.hasAttribute("physx-hidden-collision"))
          return false;
  ...
  resetBodyPose() {
    this.rigidBody.setGlobalPose(
      PhysXUtil.object3DPhysXTransform(this.el.object3D),
      true
    );
  },
  kinematicMove() {
    this.rigidBody.setKinematicTarget(
      PhysXUtil.object3DPhysXTransform(this.el.object3D)
    );
  },
  tock(t, dt) {
    ...
    if (
      this.rigidBody &&
      (this.data.type === "kinematic" || this.el.is("grabbed"))
    ) {
...
AFRAME.registerComponent("physx-joint-driver", {
  ...
  schema: {
    ...
    forceLimit: { default: 3.4028234663852885981170418348452e38 },
  ...
  setJointDriver() {
    ...
    this.joint.setDriveVelocity(
      this.data.linearVelocity,
...
AFRAME.registerComponent("physx-joint-constraint", {
  ...
  setJointConstraint() {
    ...
    let llimit = () => {
      let l = new PhysX.PxJointLinearLimitPair(
        new PhysX.PxTolerancesScale(),
        this.data.linearLimit.x,
        this.data.linearLimit.y
      );
    ...
    for (let axis of this.constrainedAxes) {
      if (
        axis === PhysX.PxD6Axis.eX ||
        axis === PhysX.PxD6Axis.eY ||
        axis === PhysX.PxD6Axis.eZ
      ) {
      ...
      if (axis === PhysX.eTWIST) {
        ...
        let pair = new PhysX.PxJointAngularLimitPair(
          this.data.limitTwist.x,
          this.data.limitTwist.y
        );
      ...
      joint.setMotion(axis, PhysX.PxD6Motion.eLIMITED);
      let cone = new PhysX.PxJointLimitCone(
        this.data.limitCone.x,
        this.data.limitCone.y
      );
  ...
  update(oldData) {
    ...
    this.constrainedAxes = PhysXUtil.axisArrayToEnums(
      this.data.constrainedAxes
    );
...
AFRAME.registerComponent("physx-joint", {
  ...
  schema: {
    ...
    type: {
      default: "Spherical",
      oneOf: ["Fixed", "Spherical", "Distance", "Revolute", "Prismatic", "D6"],
    },
  ...
  update() {
    ...
    this.joint.setConstraintFlag(
      PhysX.PxConstraintFlag.eCOLLISION_ENABLED,
      this.data.collideWithTarget
    );
      ...
      case "D6":
        {
          if (this.data.softFixed) {
            ...
            this.joint.setMotion(
              PhysX.PxD6Axis.eSWING1,
              PhysX.PxD6Motion.eFREE
            );
            this.joint.setMotion(
  ...
  getTransform(el) {
    Util.positionObject3DAtTarget(this.worldHelperParent, el.object3D, {
      scale: this.targetScale,
    });

    Util.positionObject3DAtTarget(this.worldHelper, this.el.object3D, {
      scale: this.targetScale,
    });
  ...
  async createJoint() {
    ...
    this.joint = PhysX[`Px${this.data.type}JointCreate`](
      this.system.physics,
      this.bodyEl.components["physx-body"].rigidBody,
      thisTransform,
      this.data.target.components["physx-body"].rigidBody,
      targetTransform
    );
...
AFRAME.registerComponent("gltf-entities", {
  ...
  setupObject(obj3d, currentRootEl) {
    if (obj3d.userData["a-entity"]) {
      ...
      if (
        this.data.autoPropogateGrab &&
        this.el.classList.contains("clickable")
      ) {
