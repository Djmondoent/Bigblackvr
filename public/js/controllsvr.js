...
AFRAME.registerComponent("smooth-locomotion", {
  ...
  init: function () {
    ...
    controllerL.addEventListener("thumbstickdown", () => {
      this.thumbstickPressed = true;
    });
    controllerL.addEventListener("thumbstickup", () => {
      this.thumbstickPressed = false;
    });
  ...
  handleHead: function () {
    ...
    this.player.setAttribute("ammo-shape", {
      ...
      offset: `${this.headPos.x - this.player.object3D.position.x} ${
        this.headPos.y / 2
      } ${this.headPos.z - this.player.object3D.position.z}`,
    });
  },
});
...
AFRAME.registerComponent("turn-controls", {
  ...
  init: function () {
    ...
    this.invalid =
      this.data.turnType != "snap" && this.data.turnType != "smooth";
    if (this.invalid) {
      console.log(
        "You have not entered a valid turnType! Only none, snap, and smooth are accepted."
      );
      return;
    }
  ...
  posAdjust: function () {
    ...
    if (this.posAdjustDelayed) {
      ...
      this.newPlayerPos.addVectors(
        this.player.object3D.position,
        this.headOffset
      );

      this.btNewPlayerPos.setValue(
        this.newPlayerPos.x,
        this.newPlayerPos.y,
        this.newPlayerPos.z
      );
    ...
    this.el.addEventListener("buttondown", (e) => {
      if (e.detail.id == 1 && this.closestGrabbable != null) {
        ...
        if (e.srcElement.id === "controllerL")
          this.currentGrabbable.setAttribute(
            "ammo-constraint",
            "target: #controllerL"
          );
        else
