!(function(t) {
  ...
  function(t, e) {
    ...
    AFRAME.registerComponent("alongpath", {
      ...
      getI_: function(t, e, i) {
        var r = 0;
        return (r = t - e >= i ? 1 : t - e < 0 ? 0 : (t - e) / i);
      },
      tick: function(t, e) {
        var i = this.curve.components.curve
          ? this.curve.components.curve.curve
          : null;
        if (i) {
          if (!this.el.is("endofpath")) {
            this.interval = this.interval + e;
            var r = this.getI_(this.interval, this.data.delay, this.data.dur);
            if (this.data.loop === !1 && r >= 1)
              this.el.setAttribute("position", i.points[i.points.length - 1]),
                this.el.removeState("moveonpath"),
                this.el.addState("endofpath"),
                this.el.emit("movingended");
            else if (this.data.loop === !0 && r >= 1)
              this.el.emit("movingended"), (this.interval = this.data.delay);
            else {
              this.el.is("moveonpath") ||
                (this.el.addState("moveonpath"), this.el.emit("movingstarted"));
              var a = i.getPoint(r);
              this.el.setAttribute("position", a);
            }
            if (this.data.rotate === !0) {
              var s = this.interval + e,
                o = i.getPoint(this.getI_(s, this.data.delay, this.data.dur));
              this.el.object3D.lookAt(o);
            }
            this.triggers &&
      ...
      play: function() {
        this.data.resetonplay && this.reset();
      },
      remove: function() {
      ...
      updateActiveTrigger: function() {
        for (var t = 0; t < this.triggers.length; t++)
          if (this.triggers[t].object3D) {
            if (
              this.triggers[t].object3D.position.distanceTo(
                this.el.object3D.position
              ) <= this.data.triggerRadius
            ) {
              this.activeTrigger && this.activeTrigger != this.triggers[t]
                ? (this.activeTrigger.removeState("alongpath-active-trigger"),
                  this.activeTrigger.emit("alongpath-trigger-deactivated"),
                  (this.activeTrigger = this.triggers[t]),
