  ...
  describe("when asked for a default sync connection", function() {
    ...
    before(function(done) {
      altspace.utilities.sync
        .connect({ authorId: "AltspaceVR", appId: "SyncTest" })
        .then(function(s) {
          connection = s;
          done();
        });
    ...
    it("should have a instance ref", function() {
      expect(connection)
        .to.have.a.property("instance")
        .which.is.a("Object");
      expect(connection.instance.parent().key()).to.equal(
        connection.app.child("instances").key()
      );
    });

    it("should have a app ref", function() {
      expect(connection)
        .to.have.a.property("app")
        .which.is.a("Object");
    });
    ...
    if (altspace.inClient) {
      it("should have a space ref", function() {
        expect(connection)
          .to.have.a.property("space")
          .which.is.a("Object");
        expect(connection.space.parent().key()).to.equal(
          connection.app.child("spaces").key()
        );
      });
      ...
      it("should have a user ref", function() {
        expect(connection)
          ...
          .which.is.a("Object");
        expect(connection.user.parent().key()).to.equal(
          connection.app.child("users").key()
        );
      });
    } else {
