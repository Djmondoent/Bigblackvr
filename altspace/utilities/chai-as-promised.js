  ...
  function chaiAsPromised(chai, utils) {
    ...
    function assertIsAboutPromise(assertion) {
      ...
      if (isJQueryPromise(assertion._obj)) {
        throw new TypeError(
          "Chai as Promised is incompatible with jQuery's thenables, sorry! Please use a " +
    ...
    method("rejectedWith", function(Constructor, message) {
      ...
      var derivedPromise = getBasePromise(that).then(
        function(value) {
          ...
          if (Constructor) {
            assertionMessage =
              "expected promise to be rejected with #{exp} but it was fulfilled with " +
              "#{act}";
            expected = constructorName;
          } else if (message) {
            var verb = message instanceof RegExp ? "matching" : "including";
            assertionMessage =
              "expected promise to be rejected with an error " +
              verb +
              " #{exp} but it " +
          ...
          assertIfNotNegated(that, assertionMessage, {
            expected: expected,
            actual: value
          });
        },
        function(reason) {
          ...
          var reasonMessage =
            utils.type(reason) === "object" && "message" in reason
              ? reason.message
              : "" + reason;
          if (
            message &&
            ...
            if (typeof message === "string") {
              that.assert(
                reasonMessage.indexOf(message) !== -1,
    ...
    var originalAssertMethods = Object.getOwnPropertyNames(assert).filter(
      function(propName) {
    ...
    assert.isRejected = function(promise, toTestAgainst, message) {
      ...
      return toTestAgainst !== undefined
        ? assertion.to.be.rejectedWith(toTestAgainst)
