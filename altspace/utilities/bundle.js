(function(THREE, altspace) {
  ...
  BrushBehavior.prototype.spawnObject = function spawnObject(
    position,
    rotation,
    scale
  ) {
    this._sceneSync.instantiate(
      "clone",
      {
        position: position,
        rotation: rotation,
  ...
  BrushBehavior.prototype.clearAllMyObjects = function clearAllMyObjects() {
    ...
    myObjects.forEach(function(obj) {
      return this$1._sceneSync.destroy(obj);
    });
  };
  ...
  BrushBehavior.prototype.update = function update() {
    ...
    if (controller) {
      ...
      var triggerPressed = this._triggerPressed(
        controller.buttons[SteamVRInputBehavior$1.BUTTON_TRIGGER].pressed
      );
      var gripPressed = this._gripPressed(
        controller.buttons[SteamVRInputBehavior$1.BUTTON_GRIP].pressed
      );

      var s =
        (BRUSH_SIZE / 10) * this._pixelsPerMeter +
        controller.axes[SteamVRInputBehavior$1.AXIS_TOUCHPAD_Y];
  ...
  var SteamVRTrackedObjectBehavior =
  ...
  function createClone(ref) {
    ...
    var group = createAxis();
    group.addBehaviors(new altspace.utilities.behaviors.Object3DSync());
  ...
  altspace.utilities.sync.connect(config).then(function(connection) {
    var sceneSync = new altspace.utilities.behaviors.SceneSync(
      connection.instance,
      {
        instantiators: {
          clone: createClone
    ...
    sim.scene.addBehaviors(new SteamVRInputBehavior(), sceneSync);
